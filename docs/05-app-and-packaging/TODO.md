...
7. Create a chart using the `helm create todo-app` command and delete unnecessary default files.
...
14. Check the chart using the `helm lint helm/todo-app` command (there should be no errors).
15. Run the `helm template todo-app helm/todo-app -f helm/todo-app/values-dev.yaml` command to verify the manifest output.