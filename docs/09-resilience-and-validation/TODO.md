...
3. Run the script `curl -sf http://gopher/health || exit 1` inside the hook container to check the health of the application.
...
8. Run the command `helm history gopher-prod -n gopher-prod` in the cluster to check the release history and changes.
